...
if (data.codetype === 'retargeting') {
  ...
  if (queryPermission('send_pixel', url)) {
    sendPixel(url, () => {
      ...
      data.gtmOnFailure();
      log('SKLIK RETARGETING: status failure', params, url);
    });  
  } else {
    data.gtmOnFailure();
    log('SKLIK RETARGETING: status failure: request not allowed', params, url);
  }
  ...
  if (queryPermission('send_pixel', url)) {
    ...
    data.gtmOnFailure();
    log('SKLIK CONVERSION: status failure: request not allowed', url, {'id': data.id, 'params': params});
  }
